...
public class AstConnection implements ManagerEventListener{
 ...
 //  public static SyncEntry SynData = new SyncEntry();
   ...
   public static List<String> exeCommand(String cmd)
   {
	   ...
	   CommandResponse response = (CommandResponse) managerConnection.sendAction(commandAction);  // send the cli command and wait for the response
   ...
   public static String getVariable(String chann, String var)
   {
	   String value = "";
	   try{
	   GetVarAction getVarAction = new GetVarAction(chann, var); // getting variable action
	   ManagerResponse response = managerConnection.sendAction(getVarAction);  // send the action to asterisk
   ...
   public static String getGlVariable(String var)
   {
	   String value = "";
	   try{
	   GetVarAction getVarAction = new GetVarAction(var); // getting variable action
	   ManagerResponse response = managerConnection.sendAction(getVarAction);  // send the action to asterisk
	   value = response.getAttribute("Value");  // get the variable value
	   }catch(Exception e)
	   {
   ...
   public static void setVariable(String chann, String var, String varvalue)
   {
	   ...
	   try{
	   SetVarAction setVarAction = new SetVarAction(chann, var, varvalue);  // set variable action
	   ManagerResponse response = managerConnection.sendAction(setVarAction);  // send the action to asterisk
   ...
   public static void setGlVariable(String var, String varvalue)
   {
	   ...
	   try{
	   SetVarAction setVarAction = new SetVarAction(var, varvalue);  // set variable action
	   ManagerResponse response = managerConnection.sendAction(setVarAction);  // send the action to asterisk
   ...
   // set the connection to asterisk
	public  void SetConnection () throws IOException, TimeoutException, AuthenticationFailedException
	{
		
		
		
		// connection factory
		ManagerConnectionFactory factory = new ManagerConnectionFactory(heightsReMain.cfg.asthost,
				heightsReMain.cfg.username, heightsReMain.cfg.password);
   	...
   	if (arg0 instanceof DialEvent)
    	{
    		...
    		if (dstchann.startsWith(heightsReMain.cfg.astChann))
    		{
    		    ...
    		    if (!srcchann.startsWith(heightsReMain.cfg.astChann))
    			  (new ThrSetVar(srcchann, "__DYNAMIC_FEATURES", "legalfeature")).start();
    			//	SynData.updateStartDialvalue(de.getUniqueId(), de.getDateReceived());
    ...
    /// for process dial event		
    	...
    	if (arg0 instanceof HangupEvent)
    	{
    		...
    		if (chann.startsWith(heightsReMain.cfg.astChann)||chann.startsWith("Transfered/" + heightsReMain.cfg.astChann))
    		{
    	...
    	if (arg0 instanceof DtmfEvent)
    	{
    		
    		DtmfEvent de = (DtmfEvent) arg0;
    		new dtmfEventThread(de).start();
